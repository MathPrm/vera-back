...
if (dbUrl) {
  ...
  // --- CAS LOCAL (PC / POSTGRES) ---
  console.log("💻 Mode Local : Connexion à PostgreSQL sur le port", process.env.DB_PORT);

  sequelize = new Sequelize(
    process.env.DB_NAME as string,     // On utilise directement process.env pour être sûr
    process.env.DB_USER as string,
    process.env.DB_PASSWORD as string,
    {
      host: process.env.DB_HOST,
      port: Number(process.env.DB_PORT) || 5432, // <--- LIGNE MAGIQUE À AJOUTER
      dialect: 'postgres',
      logging: console.log
    }
  );

...
const tiktok_service_js_1 = __importDefault(require("./tiktok.service.js"));
const youtube_service_js_1 = __importDefault(require("./youtube.service.js"));
const instagram_service_js_1 = __importDefault(require("./instagram.service.js"));
class VeraService {
    constructor() {
        ...
        this.tiktokService = tiktok_service_js_1.default;
        this.youtubeService = youtube_service_js_1.default;
        this.instagramService = instagram_service_js_1.default;
...
Réponds avec un verdict: VERIFIED, MOSTLY_TRUE, MIXED, MOSTLY_FALSE, ou FALSE`;
    ...
    parseVeraResponse(veraData, videoData) {
        ...
        const toolsUsed = [];
        if (lowerResponse.includes('deepfake'))
            toolsUsed.push('Détection deepfake');
        if (lowerResponse.includes('synthetic') || lowerResponse.includes('synthétique'))
            toolsUsed.push('Détection contenu IA');
        return {
    ...
    async checkContent(message, conversationId = null, conversationHistory = [], mediaUrls = [], imageFile = null, videoFile = null) {
        try {
            ...
            const platformVideos = extractedMedias.filter(m => ['tiktok', 'youtube', 'instagram'].includes(m.platform));
...
🎯 TÂCHE: Vérifie les informations factuelles dans ce contenu.`;
            ...
            if (conversationHistory.length > 0) {
                ...
                const context = lastMessages.map(msg => `${msg.sender === 'user' ? 'Utilisateur' : 'Vera'}: ${msg.content}`).join('\n');
    ...
    calculateConfidence(response, sourcesCount) {
        ...
        if (response.length > 200)
            confidence += 10;
        if (response.match(/selon|d'après|source|étude/gi))
            confidence += 10;
        return Math.min(confidence, 95);
    }
}
exports.default = new VeraService();
//# sourceMappingURL=vera.service.js.map